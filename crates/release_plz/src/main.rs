...
async fn run(args: CliArgs) -> anyhow::Result<()> {
    match args.command {
        ...
        Command::ReleasePr(cmd_args) => {
            // Because we are creating a PR, we'll need to be able to access the remote repository
            // The git token is the best method to do this at the moment
            anyhow::ensure!(
            ...
            // The config file contains a bunch of parameters that change the behavior of the
            // update PR command.
            ...
            // In the case we are working in a workspace and multiple packages are updated, there
            // is still only a single PR that updates all of the package manifests as far as I am
            // aware.
